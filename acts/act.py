...
def imgB64decode(imgB64):
	...
	pattern=re.compile("^(?:[A-Za-z0-9+/]{4})*(?:[A-Za-z0-9+/]{2}==|[A-Za-z0-9+/]{3}=|[A-Za-z0-9+/]{4})$")
	if pattern.match(imgB64):
		return 1
	else:
		return 0
...
def addcategory():
	...
	if request.method == "GET":
		...
		if checkCategoryInDb(category_data[0]):
			...
			cursor.execute("INSERT INTO Category(categoryname) VALUES(?)", (category_data[0]))
			connectionState.commit()
			connectionState.close()
			return jsonify({}), 201
		else:
...
def actsforcategory(category):
	...
	if start==None and end==None:
		...
		if request.method=='GET':
			...
			mydic={}
			actlist=[]
			if checkCategory(category) or checkactsno(category):
				return jsonify({}),204
...
def upload():
	...
	if request.method == 'POST':
		...
		connectionState=sqlite3.connect('acts_databs.db')
		cursor = connectionState.cursor()
		r=requests.get(url="http://34.260.204.149:80/api/v1/users")
		users=r.json()
		print(users)
		#for i in users:
		#	if i==
		...
		for i in users:
			if(i==act_data['username']):
				if checkactid(act_data['actId']) and checkcategoryname(act_data['categoryName']) and timecheck(act_data['timestamp']) and imgB64decode(act_data['imgB64']) and validate_request(act_data,dict,6)==False and set(act_data.keys())!=validate_set :
					act_data['upvotes']=0
...
@app.route('/api/v1/_health' , methods=['GET','DELETE','POST','PUT'])
def health():
	global health
	if request.method=='GET':
		connectionState=sqlite3.connect('acts_databs.db')
...
@app.route('/api/v1/_crash' , methods=['GET','DELETE','POST','PUT'])
def crash():
	global health
	if request.method=='POST':
		connectionState=sqlite3.connect('acts_databs.db')
